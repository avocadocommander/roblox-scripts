...
import { getRandomMedievalPhrase, isNPCType, MEDIEVAL_NAMES, NPCType, useAssetId } from "shared/module";
...
function patrol(npc: Model, routePoints: BasePart[]) {
	...
	const startPatrol = () => {
		...
		humanoid.MoveToFinished.Once(() => {
			...
			task.delay(math.random(10, 60), startPatrol);
		});
	};
...
function getClosestSpawnPointRelativeToRoute(firstRoutePointToCompare: BasePart): BasePart {
